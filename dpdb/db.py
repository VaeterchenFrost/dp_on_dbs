...
class DB(object):
    ...
    def insert_select(self, table, select, columns=None, returning=None):
        sql_str = "INSERT INTO {} {}"
        q = sql.SQL(sql_str).format(
            self.__table_name__(table), sql.SQL(select))
        if columns:
            q = sql.Composed(
                [q, sql.SQL(', ').join(map(sql.Identifier, columns))])
        if returning:
            q = sql.Composed(
                [q, sql.SQL(" RETURNING {}").format(sql.Identifier(returning))])
    ...
    def select(self, table, columns, where=None):
        ...
        if where:
            q = sql.Composed([q, sql.SQL(" WHERE {}").format(
...
class DBAdmin(DB):
    def killall(self, app_name):
        ...
        """
        q = "select pg_terminate_backend(pid) from pg_stat_activity where pid \
            <> pg_backend_pid() and datname = %s"
        if app_name:
