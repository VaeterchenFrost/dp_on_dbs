...
class CnfReader(DimacsReader):
    ...
    def store_problem_vars(self):
        ...
        if self.problem_solution_type == 's':
            logger.info(
                "Problem has %d models (solved by pre-processing)",
                int(self.format))
        else:
    ...
    def body(self, lines):
        ...
        for lineno, line in enumerate(lines):
            ...
            while line[-1] != '0':
                if lineno + i >= len(lines):
                    logger.warning(
                        "Clause at line %d not terminated with 0", lineno)
        ...
        if maxvar != self.num_vars:
            logger.warning(
                "Effective number of variables mismatch preamble (%d vs %d)",
                maxvar,
                self.num_vars)
        if len(self.clauses) != self.num_clauses:
            logger.warning(
                "Effective number of clauses mismatch preamble (%d vs %d)",
                len(self.clauses), self.num_clauses)
...
class TdReader(DimacsReader):
    ...
    def store_problem_vars(self):
        if self.problem_solution_type == "p":
            ...
            logger.error(
                "Unrecognized problem or solution indicator: %s",
                self.problem_solution_type)
    ...
    def body(self, lines):
        ...
        for lineno, line in enumerate(lines):
            ...
            if self.is_comment(line):
                ...
                _add_directed_edge(
                    self.edges, self.adjacency_list, vertex1, vertex2)
                _add_directed_edge(
                    self.edges, self.adjacency_list, vertex2, vertex1)

        if self.problem_solution_type == "p":
            if len(self.edges) != self.num_edges * 2:
                logger.warning(
                    "Effective number of edges mismatch preamble (%d vs %d)",
                    len(self.edges) / 2, self.num_edges)
        elif self.problem_solution_type == "s":
            if len(self.bags) != self.num_bags:
                logger.warning(
                    "Effective number of bags mismatch preamble (%d vs %d)",
                    len(self.bags), self.num_bags)
