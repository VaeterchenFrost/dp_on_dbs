...
from dpdb.db import (BlockingThreadedConnectionPool,
                     DEBUG_SQL, setup_debug_sql, DBAdmin)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        usage="%(prog)s [general options] -f input-file problem-type "
    ...
    problem_parsers = parser.add_subparsers(
        ...
        "problem-type --help for additional information on each type "
        "and problem specific options",
    ...
    for cls, prob_args in args.specific.items():
        ...
        p = problem_parsers.add_parser(
            cls.__name__, **prob_args, usage="%(prog)s")
    ...
    parser.add_argument(
        ...
        "--file",
        dest="file",
        help="Input file for the problem to solve",
        required=True)
    ...
    gen_opts.add_argument(
        "-t",
        dest="type",
        help="type of the cluster run",
        default="")
    gen_opts.add_argument(
        "--runid",
        dest="runid",
        help="runid of the cluster run",
        default=0,
        type=int)
    gen_opts.add_argument(
        "--config",
    ...
    gen_opts.add_argument(
        "--td-file",
        dest="td_file",
        help="Store TreeDecomposition file (htd Output)")
    gen_opts.add_argument(
        "--gr-file",
        dest="gr_file",
        help="Store Graph file (htd Input)")
    gen_opts.add_argument(
        "--faster",
        dest="faster",
        help="Store less information in database",
        action="store_true")
    gen_opts.add_argument(
        "--parallel-setup",
        dest="parallel_setup",
        help="Perform setup in parallel",
        action="store_true")

    # problem options
    prob_opts = parser.add_argument_group(
        "problem options",
        "Options that apply to all problem types")
